...
export const EXEC_CAPABLE_TOOL_NAMES: ReadonlySet<string> = new Set([
  ...
  "cmd",
  "command",
  // Codex app-server dynamic-tool renames of the shell exec/process tools
  // (extensions/codex/src/app-server/shell-dynamic-tools.ts). These are NOT
  // core-tool-factory descriptors, so stripKnownFamilyPrefix's guard
  // intentionally refuses to unwrap their flattened forms — list both bare and
  // flattened names here instead. Live-drilled 2026-08-24: `openclawnode_exec`
  // carrying `echo LEGA > ~/…` fell through Tier-A to SUPERSET_EFFECTS
  // (`command:"<unparsed>"`), so the resolver could only blind-deny it — same
  // gap class as the `openclawweb_fetch` fix (0d8d6a5492). Over-matching is
  // fail-closed (worst case: an unrelated tool with this exact name gets
  // classified exec-capable and prompted, never silently allowed).
  "node_exec",
  "node_process",
  "openclawnode_exec",
  "openclawnode_process",
]);
