...
describe("classifyTierA", () => {
  ...
  it("codex node_exec renames (bare + flattened) → [process.exec] with the command VISIBLE", () => {
    // Live-drilled 2026-08-24: codex's node_exec dynamic tool (a rename of the
    // shell exec tool) reached the classifier as `openclawnode_exec`, missed
    // every Tier-A table, and fell to SUPERSET_EFFECTS — handing the resolver
    // an opaque `command:"<unparsed>"` it could only blind-deny. Same class as
    // the `openclawweb_fetch` gap (0d8d6a5492), but the strip guard can't
    // unwrap these (node_exec is not a core-tool-factory descriptor), so the
    // names are listed in EXEC_CAPABLE_TOOL_NAMES directly.
    const cmd = "echo LEGA > /Users/x/sigil-drill-legA-1";
    for (const name of ["node_exec", "openclawnode_exec"]) {
      const result = classifyTierA(null, name, { command: cmd });
      expect(result).toHaveLength(1);
      expect(result[0]?.kind).toBe("process.exec");
      expect(result[0]?.command).toBe(cmd);
      expect(result[0]?.unparseable).toBeUndefined();
    }
    for (const name of ["node_process", "openclawnode_process"]) {
      const result = classifyTierA(null, name, {});
      expect(result).toHaveLength(1);
      expect(result[0]?.kind).toBe("process.exec");
    }
    // The guard still refuses to invent exec-capability for OTHER flattened
    // names — no silent rewrite of arbitrary openclaw-prefixed tools.
    expect(classifyTierA(null, "openclawnode_foo", {})).toEqual([]);
  });
