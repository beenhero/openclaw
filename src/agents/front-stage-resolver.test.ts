...
// ---------------------------------------------------------------------------
// Consult-once per toolCallId (grant-dedupe slice, 2026-08-24). One codex tool
// call must not consult the resolver twice: the node_exec wrapper's inner exec
// delegate re-enters with the SAME toolCallId (+ identical params). Dedupe key
// = toolCallId + effects digest — only a byte-identical intent shares a verdict.
// ---------------------------------------------------------------------------
describe("consult-once per toolCallId", () => {
  it("same toolCallId + identical params (renamed tool) → ONE consult, shared deny", async () => {
    let consults = 0;
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
        return { requestId: req.requestId, decision: "deny", reason: "shared-deny" };
      }),
    );
    const base = {
      params: { cmd: "rm -rf /x" },
      ctx: makeCtx({ config: undefined }),
      signal: AbortSignal.timeout(5000),
      toolCallId: "call_dedupe_1",
    };
    const r1 = await runFrontStageResolver({ ...base, toolName: "bash" });
    const r2 = await runFrontStageResolver({ ...base, toolName: "openclawnode_exec" });
    expect(consults).toBe(1);
    expect(r1?.blocked).toBe(true);
    expect(r2?.blocked).toBe(true);
  });

  it("same toolCallId, ALLOW verdict is shared too (both undefined, one consult)", async () => {
    let consults = 0;
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
    ...
    const base = {
      ...
      ctx: makeCtx({ config: undefined }),
      signal: AbortSignal.timeout(5000),
      toolCallId: `call_dedupe_2_${Date.now()}_${Math.random().toString(36).slice(2)}`,
    };
    const r1 = await runFrontStageResolver({ ...base, toolName: "bash" });
    const r2 = await runFrontStageResolver({ ...base, toolName: "exec" });
    expect(consults).toBe(1);
    expect(r1).toBeUndefined();
    expect(r2).toBeUndefined();
  });

  it("different toolCallId → consulted per call", async () => {
    let consults = 0;
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
        return { requestId: req.requestId, decision: "deny", reason: "d" };
      }),
    );
    const base = {
      params: { cmd: "rm -rf /x" },
    ...
    await runFrontStageResolver({ ...base, toolName: "bash", toolCallId: "call_dedupe_3a" });
    await runFrontStageResolver({ ...base, toolName: "bash", toolCallId: "call_dedupe_3b" });
    expect(consults).toBe(2);
  });

  it("same toolCallId but DIFFERENT params (different digest) → consulted per call", async () => {
    let consults = 0;
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
    ...
    const base = {
      ctx: makeCtx({ config: undefined }),
      signal: AbortSignal.timeout(5000),
      toolCallId: "call_dedupe_4",
    };
    await runFrontStageResolver({ ...base, toolName: "bash", params: { cmd: "rm -rf /x" } });
    await runFrontStageResolver({
      ...base,
      toolName: "bash",
      params: { cmd: "rm -rf /DIFFERENT" },
    });
    expect(consults).toBe(2);
  });

  it("no toolCallId → no dedupe", async () => {
    let consults = 0;
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
    ...
    await runFrontStageResolver({ ...base, toolName: "bash" });
    await runFrontStageResolver({ ...base, toolName: "bash" });
    expect(consults).toBe(2);
  });

  it("CONCURRENT same-key calls attach to one in-flight consult", async () => {
    let consults = 0;
    let release!: () => void;
    ...
    setActivePluginRegistry(
      makeRegistryWithResolver("process.exec", async (req) => {
        consults += 1;
        await gatePromise;
        return { requestId: req.requestId, decision: "deny", reason: "slow-deny" };
      }),
    );
    const base = {
      params: { cmd: "rm -rf /x" },
      ctx: makeCtx({ config: { approvals: { frontStageResolverTimeoutMs: 5000 } } }),
      signal: AbortSignal.timeout(5000),
      toolCallId: "call_dedupe_5",
    };
    const p1 = runFrontStageResolver({ ...base, toolName: "bash" });
    await new Promise((r) => setTimeout(r, 10));
    const p2 = runFrontStageResolver({ ...base, toolName: "openclawnode_exec" });
    await new Promise((r) => setTimeout(r, 10));
    release();
