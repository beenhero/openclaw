...
export async function runFrontStageResolver(args: {
  ...
  // --- 3b. CONSULT-ONCE per toolCallId (grant-dedupe slice, 2026-08-24) ---
  // One codex tool call must not consult the resolver twice: the node_exec
  // wrapper's inner exec delegate re-enters this function with the SAME
  // toolCallId and identical params (live-traced: asks at +0s and +4s sharing
  // one call id). Key = toolCallId + effects digest, so ONLY a byte-identical
  // intent shares a verdict — same call id with different params (different
  // digest) still consults. Absent toolCallId ⇒ no dedupe. The settled outcome
  // is retained briefly (TTL) for the lagging delegate; an in-flight consult is
  // shared as-is (the attacher inherits the first caller's deadline/signal —
  // acceptable: both belong to the same tool call).
  const consultKey = toolCallId ? `${toolCallId}:${digestForEffects(effects)}` : null;
  if (consultKey) {
    const hit = FRONT_STAGE_CONSULTS.get(consultKey);
    if (hit) {
      if (hit.settledAt === undefined || Date.now() - hit.settledAt < FRONT_STAGE_CONSULT_TTL_MS) {
        return hit.p;
      }
      FRONT_STAGE_CONSULTS.delete(consultKey); // expired — consult afresh
    }
  }
  const consult = () => consultResolver({ toolName, params, ctx, signal, toolCallId, effects });
  if (!consultKey) {
    return consult();
  }
  const entry: FrontStageConsultEntry = { p: undefined as never };
  entry.p = (async () => {
    try {
  ...
  FRONT_STAGE_CONSULTS.set(consultKey, entry);
  return entry.p;
}

/** Retention for settled consult outcomes — covers a delegate lagging seconds behind. */
const FRONT_STAGE_CONSULT_TTL_MS = 30_000;
type FrontStageConsultEntry = {
  p: Promise<FrontStageBlockedOutcome | undefined>;
  settledAt?: number;
};
const FRONT_STAGE_CONSULTS = new Map<string, FrontStageConsultEntry>();

/** The single-consult body: build the request, decide, map the verdict. */
async function consultResolver(args: {
  toolName: string;
  params: unknown;
  ctx: FrontStageCtx | undefined | null;
  signal: AbortSignal | undefined;
  ...
  const { toolName, params, ctx, signal, toolCallId, effects } = args;
