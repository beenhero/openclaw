...
/**
 * Deadline for the resolver decision in the ACP gateway context.
 *
 * Raised 5s → 300s (live-drilled 2026-08-24): a wallet-signing resolver is
 * human-in-the-loop — the owner reads the command on their phone and signs.
 * 5s abandoned every such approval before a human could possibly act, so the
 * resolver's Telegram prompts became orphans by construction (signatures
 * landed, nonces burned, the requesting call already dead). 300s matches the
 * codex dynamic-tool wait (CODEX_DYNAMIC_TOOL_TIMEOUT_MS) and Sigil's own
 * approval window (300s, clamped [30,300]) so the whole chain holds together.
 */
export const ACP_RESOLVER_DECISION_DEADLINE_MS = 300_000;
