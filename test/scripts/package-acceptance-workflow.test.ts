...
describe("package artifact reuse", () => {
  ...
  it("preserves the primary runtime token-efficiency failure", () => {
    ...
    const runStep = workflowStep(job, "Run live required runtime-pair lane");
  ...
  it("runs canonical runtime-pair lanes in parallel and preserves one gate", () => {
    ...
    expect(laneJob.strategy?.matrix?.lane).toContain('["required","soak"]');
    expect(laneJob.strategy?.matrix?.lane).toContain('["required"]');
