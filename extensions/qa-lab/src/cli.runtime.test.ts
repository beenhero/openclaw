...
describe("qa cli runtime", () => {
  ...
  it("rejects runtime-pair lanes with no compatible flow scenarios", async () => {
    ...
    await expect(
      ...
      "--runtime-pair-lane matched no execution.kind: flow scenarios for required; incompatible scenario(s): codex-plugin-cold-install (script).",
    );
  ...
  it("rejects unknown runtime-pair lane filters", async () => {
    await expect(
      runQaSuiteCommand({
        repoRoot: "/tmp/openclaw-repo",
        runtimePairLane: ["requiredish"],
      }),
    ).rejects.toThrow(
      '--runtime-pair-lane must be one of required, optional, soak, got "requiredish".',
    );
  });
