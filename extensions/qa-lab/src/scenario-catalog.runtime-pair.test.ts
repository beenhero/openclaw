...
describe("QA runtime-pair scenario catalog", () => {
  it("uses the canonical lanes with audited declaration counts", () => {
    expect(QA_RUNTIME_PAIR_LANES).toEqual(["required", "optional", "soak"]);
    ...
    expect(laneCounts).toEqual({ required: 39, optional: 8, soak: 2 });
  });

  it("declares every release agentic scenario in the required lane", () => {
  ...
  it("keeps runtime-pair membership independent from provider eligibility", () => {
    ...
    for (const scenarioId of liveRequiredScenarioIds) {
      expect(readQaScenarioById(scenarioId).runtimePairLane).toBe("required");
      expect(readQaScenarioExecutionConfig(scenarioId)).toMatchObject({
        requiredProviderMode: "live-frontier",
      });
    }

    expect(readQaScenarioById("gateway-restart-inflight-run").runtimePairLane).toBe("required");
