...
Extended-stable instead runs npm preflight, Full Release Validation, and plugin
...
publish binds every run to the canonical branch and release SHA. Its tag owns
immutable Docker images and only the `extended-stable*` aliases; it skips the
regular orchestrator and its ClawHub, native-app, GitHub Release, and private
dist-tag surfaces. See [Monthly extended-stable
