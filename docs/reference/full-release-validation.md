...
Do not use `pnpm ci:full-release` or `release-ci/*`: publish checks the run's
head branch/SHA, target SHA, manifest `workflowRef`, run ID, and attempt against
the canonical branch and release commit. This evidence is always fresh.

Product failures need a bounded backport; frozen-target tooling failures need
the smallest behavior-preserving compatibility repair; transient provider,
approval, or runner failures keep the commit unchanged. Any branch change needs
a new complete run. Never omit required package, installer, update, channel, or
live behavior merely because the target is old.
