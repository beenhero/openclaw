...
| ------------------------------------------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| codex × process.exec                              | **LIVE**                                    | OAuth drill on real Codex 0.145.0 (min version floor 0.143.0): deny blocked an auto-run exec, allow ran it; resolver saw `{requestId, effects:[{kind:"process.exec",...}], paramsDigest}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| codex × net.egress (curl)                         | **LIVE**                                    | OAuth drill on real Codex 0.145.0: deny blocked a real `curl` command; resolver saw `net.egress` effect with extracted hosts (`example.com`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| codex × fs.write (write cmds)                     | **LIVE**                                    | OAuth capstone drill on real Codex 0.145.0: deny blocked a real `touch <file>`; resolver saw the effect SET `[{process.exec, command}, {fs.write, paths:[<file>]}]` (one command → one decision, `pickOwner`=process.exec). Tier-C refines `touch`/`tee`/`cp`/`>`/`rm`/`mv`/`dd` to `fs.write paths`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| native × net.egress (web_fetch)                   | **LIVE (real network)**                     | Real-network drill against a live host: a `net.egress` resolver gated the REAL `web_fetch` tool (real DNS, real fetch client). Deny → front-stage short-circuited in `runBeforeToolCallHook` and the tool never ran (no request left the process; the unit test `web-fetch-net-egress-gate.test.ts` proves the fetch spy is not called on deny). Allow → 1523 chars of real content fetched from `example.com`. Enforcement AND real egress both confirmed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| native × process.exec (OpenClaw-owned exec tools) | **LIVE-enforced in-harness**                | Dedicated E2E `process-exec-native-gate.test.ts`: a `process.exec` resolver deny → the wrapped tool's `execute` spy is NOT called (block enforced); allow runs it; a failure-disposition deny throws fail-closed with the spy uncalled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| ACP server-mode (#97152 ACP-client bypass)        | **LIVE end-to-end (deny + allow)**          | **Live drill 2026-07-23** against a real seam-carrying Gateway (branch 2026.7.2, isolated home + alt port), driven by a headless programmatic ACP client → resolver-injected `serveAcpGateway` wrapper → Gateway, with a real **embedded anthropic** agent proposing an exec tool call. DENY: resolver `FIRED → DENY` on `process.exec` (`echo …`) → agent tool_call `failed "Exec denied (gateway, user-denied)"`, command never ran, ACP client's `requestPermission` NEVER called. ALLOW: resolver `FIRED → ALLOW` → tool_call `completed`, command ran, `requestPermission` NEVER called. Falsification control: same agent+command, only the resolver verdict differs → deny blocks / allow runs, both suppress the client tap. Also integration-proven independently (`translator.resolver-first.test.ts`). **Trigger surface (live-verified):** the resolver-first fires ONLY for an approval bound to an in-flight ACP prompt turn on the bridge's own session (the embedded runtime's `agent stream:"approval"` event); a `/bash` DIRECT-command approval is backend-conn-owned with no ACP pending prompt (targeted delivery excludes the ACP bridge) → routes to operator surfaces, never reaches `runApprovalRelay`. This live-verified the NEGATIVE half of the seam's own documented "dual-trigger UNVERIFIED" caveat (resolver-first.ts:56-68) — the positive half (agent-turn trigger) is now LIVE-confirmed |
